...
class RunApp(app_base.AppBase):
    ...
    def on_install(self):
        run_tab_x = 65
        M5.Lcd.drawImage(
            "/system/energybean_cores3/Selection/appRun_unselected.png", run_tab_x, 20 + 4
        )
        self.descriptor = app_base.Descriptor(x=run_tab_x, y=20 + 4, w=62, h=56)
    ...
    def on_view(self):
        run_tab_x = 65
        M5.Lcd.drawImage(
            "/system/energybean_cores3/Selection/appRun_selected.png", run_tab_x, 20 + 4
        )
        M5.Lcd.drawImage("/system/energybean_cores3/Run/run.png", 4, 20 + 4 + 56 + 4)
        ...
        # Hide account line in energybean UI.
    ...
    def _get_file_info(path) -> tuple(str, str, str):
        mtime = None
        account = None
        ver = f"Ver: {esp32.firmware_info()[3]}"
